...
export function calcTitle(
  metal: Metal,
  sortament: Sortament,
  calcType: CalcType,
): string {
  const metalWord = metal.form_title.replace(
    "{end}",
    sortament.metall_title_end,
  );
  const prefix = calcType === "length" ? "Расчет длины" : "Расчет веса";
  return `${prefix} ${metalWord} ${sortament.form_title}`;
}
